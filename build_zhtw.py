...
def protect_fixed_terms(text: str) -> tuple[str, dict[str, str]]:
    fixed_terms = [
        "#学園アイドルマスター",
        "#学マス",
        "学園アイドルマスター",
    ...
    protected = text
    for i, term in enumerate(sorted(set(fixed_terms), key=len, reverse=True)):
        token = f"__JPTERM_{i}__"
...
def restore_fixed_terms(text: str, mapping: dict[str, str]) -> str:
...
def convert_multiline_mixed_text(text: str, cc: OpenCC) -> str:
    ...
    for line in lines:
        ...
        if has_kana(stripped):
            ...
            protected, mapping = protect_fixed_terms(line)
            converted_lines.append(restore_fixed_terms(cc.convert(protected), mapping))
...
def iter_convert_json(obj: Any, cc: OpenCC) -> Any:
    ...
    if isinstance(obj, str):
        ...
        if should_preserve_single_line(obj):
            return obj
        protected, mapping = protect_fixed_terms(obj)
        return restore_fixed_terms(cc.convert(protected), mapping)
    return obj
...
def main() -> int:
    ...
    write_text(marker_file, "build_zhtw.py marker: FINAL-KANA-ONLY-V1\n")
